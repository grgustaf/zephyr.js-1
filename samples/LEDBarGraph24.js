...
console.log('Starting bar graph sample...');
...
function LEDBarGraph24(addr) {
    this.addr = addr;
    this.buffer = new Buffer(7);
    for (var i = 0; i < 7; i++) {
...
LEDBarGraph24.prototype.init = function () {
    this.setBrightness(15);
    this.updateDisplay();
}

LEDBarGraph24.prototype.setBrightness = function (level) {
...
LEDBarGraph24.prototype.setBar = function (bar, color) {
...
LEDBarGraph24.prototype.updateDisplay = function () {
    i2cDevice.write(this.addr, this.buffer);
}

graph = new LEDBarGraph24(I2C_ADDR);
graph.init();
